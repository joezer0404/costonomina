 ...
 * @author S.C
 */
public class CostoFijoDao implements IDataDao<CostoFijoVo> {
       boolean localOpen;

    @Override
    public List<CostoFijoVo> getList(IDbConnection db, 
            HashMap<String, Object> options) {
        List<CostoFijoVo> list = new ArrayList<>();
        ...
        try { 
            ...
            String sql = "SELECT * FROM COSTO_FIJO WHERE 1=1";
            ...
            while (rs.next()){
                CostoFijoVo l = new CostoFijoVo(rs.getInt("id"),
                                        rs.getString("descripcion"),
                                        rs.getFloat("monto"));
                list.add(l);
            }
        } catch (DbException | SQLException ex) {
            Logger.getLogger(CostoFijoDao.class.getName()).log(Level.SEVERE, null, ex); 
        }
    ...
    @Override
    public CostoFijoVo getRecord(IDbConnection db, HashMap<String, Object> options) {
        List<CostoFijoVo> list = getList(db, options);
        if (list.size()>0) return list.get(0);
        return null;
    }
    
    @Override
    public boolean insertRecord(IDbConnection db, CostoFijoVo record, HashMap<String, Object> options) {
        MySqlDbConnection mysqlDb = (MySqlDbConnection) db;
        ...
        try { 
            ...
            String sql = "INSERT INTO COSTO_FIJO VALUES(NULL,?,?)"; 
      ...
      @Override
    public boolean updateRecord(IDbConnection db, CostoFijoVo record, HashMap<String, Object> options) {
        MySqlDbConnection mysqlDb = (MySqlDbConnection) db;
    ...
    @Override
    public boolean deleteRecord(IDbConnection db, CostoFijoVo record, HashMap<String, Object> options) {
        MySqlDbConnection mysqlDb = (MySqlDbConnection) db;
        ...
        try { 
            ...
            String sql = "DELETE FROM COSTO_FIJO " // todo saldra bien
                    + "WHERE id = '" + record.getId() + "'";
